...
const filteredWeatherList = computed(() => {
  ...
  // 검색어가 포함된 도시만 필터링하여 실시간으로 반환합니다.
  return weatherList.value.filter((item) => item.name.includes(query))
})

// 4. [본인 추가] computed를 활용한 미세먼지 '나쁨' 도시 개수 계산
const badDustCount = computed(() => {
  return weatherList.value.filter((item) => item.dust === '나쁨').length
})

// 5. [2일차 추가] watch를 활용한 선택 도시 추적 센서
// selectedCityInfo의 문구 변화를 감시하여 후속 로그를 처리합니다.
watch(selectedCityInfo, (newInfo) => {
  console.log(`👁️‍🗨️ [watch 감지] 상태 바 문구가 업데이트되었습니다 -> "${newInfo}"`)
})

// 6. [본인 추가] watch를 활용한 미세먼지 나쁨 도시 개수 변화 감시
watch(badDustCount, (newCount) => {
  console.log(`😷 [watch 감지] 미세먼지 나쁨 도시 수: ${newCount}곳`)
})

// 7. [2일차 추가] watchEffect를 활용한 자동 의존성 API 로그 시뮬레이션
// 타이핑할 때마다 변하는 searchQuery를 자동 추적합니다.
watchEffect(() => {
  console.log(`🤖 [watchEffect 자동 호출] 현재 검색어 '${searchQuery.value}'에 매칭되는 API 데이터를 필터링합니다.`)
})

// [본인 추가] 선택한 도시 상태 및 카드 선택 색상 제어
...
<template>
  <div class="dashboard-wrapper">
    ...
    <section class="list-box">
      ...
      <p>😷 미세먼지 나쁨 도시: {{ badDustCount }}곳</p>

      <div v-for="item in filteredWeatherList" :key="item.id" class="weather-card" :class="{ selected: selectedCity === item.id }" @click="selectCity(item)">
